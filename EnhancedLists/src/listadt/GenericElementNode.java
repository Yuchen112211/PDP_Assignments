...
public class GenericElementNode<T> implements GenericListADTNode<T> {
  ...
  @Override
  public GenericListADTNode reverse(GenericListADTNode next) {
    T current_val = this.object;
    GenericElementNode new_node = new GenericElementNode(current_val,next);
    return this.rest.reverse(new_node);
  }

  @Override
  public GenericListADTNode interleave(GenericListADTNode add, int count) {
    if (count % 2 == 0) {
      return new GenericElementNode(this.object, this.rest.interleave(add, count + 1));
    }
    else {
      return this.rest.interleave(add, count + 1);
    }
  }
