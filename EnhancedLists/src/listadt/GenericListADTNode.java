...
public interface GenericListADTNode<T> {
  ...
  /**
   * Reverse the linked list of this node.
   * @return the linked list head after the reverse.
   */
  GenericListADTNode reverse(GenericListADTNode next);

  /**
   * This method changes this list to be a list that contains elements
   * from the original list at even positions, followed by odd positions.
   * @param add The node to be added at the end.
   * @param count In order to determine which node to be added into.
   * @return return the re-format node.
   */
  public GenericListADTNode interleave(GenericListADTNode add, int count);
